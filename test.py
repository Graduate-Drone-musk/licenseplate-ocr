...
from util import CTCLabelConverter, AttnLabelConverter, Averager
...
def test(opt):
    ...
    os.makedirs(f'./test_model/{opt.exp_name}', exist_ok=True)
    os.system(f'cp {opt.saved_model} ./test_model/{opt.exp_name}/')
    ...
    with torch.no_grad():
        if opt.benchmark_all_eval:  # evaluation with 10 benchmark evaluation datasets
            ...
            log = open(f'./test_model/{opt.exp_name}/log_evaluation.txt', 'a')
            AlignCollate_evaluation = AlignCollate(imgH=opt.imgH, imgW=opt.imgW, keep_ratio_with_pad=opt.PAD)
...
if __name__ == '__main__':
    lmdb_dir = 'C:\\Users\\parks\\Desktop\\carplate_detect\\easy_ocr\\lmdb'
    train_data = os.path.join(lmdb_dir, "training")
    valid_data = os.path.join(lmdb_dir, "validation")
    select_data= "{}-{}".format("CI", "RI")
    batch_ratio= "{}-{}".format("0.5", "0.5")
    
    path = "C:\\Users\\parks\\Desktop\\carplate_detect\\easy_ocr\\saved_models"
    
    input_channel = "1"
    output_channel = "256"
    hidden_size="256"

    character_set = "0123456789:가거고구나너노누다더도두라러로루마머모무바버보부사서소수아어오우자저조주하허호배공해육합국울인천대전충북남강원"
    
    parser = argparse.ArgumentParser()
    parser.add_argument('--eval_data', required=False, help='path to evaluation dataset', default=valid_data)
    ...
    parser.add_argument('--FeatureExtraction', type=str, required=False, help='FeatureExtraction stage. VGG|RCNN|ResNet', default="VGG")
    parser.add_argument('--SequenceModeling', type=str, required=False, help='SequenceModeling stage. None|BiLSTM', default="BiLSTM")
